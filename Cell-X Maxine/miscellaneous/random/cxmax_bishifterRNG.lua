name = "CX-Max Random Bishifter"
id = "CXMAX_BISHIFTERRNG"
author = "traincrisis"
texture = "cxmax_bishifterRNG.png"
updateIndex = 14000
...
function Step(this)
    ...
    local randomChoice = GetRandomDirection() -- Randomly choose between 0 (both), 1 (right), or 2 (left)
    ...
    if cellBehind.name ~= "EMPTY" then
        ...
        -- Delete the original cell behind the Bishifter after shifting
        cellBehind.Delete()
    end

    -- Step 2: Pull cells from 2 cells away behind the Bishifter
    local function PullCellsBehind(this, dir)
        local oppositeDir = (dir + 2) % 4

        -- Ensure there's an empty cell right next to the Bishifter
